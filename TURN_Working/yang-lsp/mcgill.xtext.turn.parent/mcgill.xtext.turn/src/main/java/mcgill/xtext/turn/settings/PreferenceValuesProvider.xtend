package mcgill.xtext.turn.settings
...
class PreferenceValuesProvider implements IPreferenceValuesProvider {
	...
	protected def IPreferenceValues createPreferenceValues(Resource resource) {
		...
		val userSettings = userHome.resolve(".turn").resolve("turn.settings")
		...
		if (config.path !== null) {
			...
			val workspaceSettings = Paths.get(workspaceDirectory).resolve("turn.settings")
			...
			val projectSettings = Paths.get(projectDirectory).resolve("turn.settings")
